...
repo as it's transferred to NCSA. ~ ~ ~ A suggestion of the RSP be
installed at SLAC so that images can be retrieved quickly and viewed. ~
The data at SLAC must be moved to a node that is accessible to outside
an outside internet connection able to reach NCSA. ~ The data is then
...
~ Those are 2 suggestions moving forward with these tests once the CCOB
is on the mountain.~ Depending on how long the CCS environment stays at
SLAC, depends on how much effort needs to go into making these transfer
mechanisms faster or not. ~\\[2\baselineskip]
